...
resource "azurerm_network_security_group" "app-nsg" {
    ...
    security_rule {
        name = "ssh-rule-2"
        priority = 101
        direction = "Outbound"					#allow app-web
        access = "Allow"
        protocol = "Tcp"
        source_address_prefix = "192.168.1.0/24"		
        source_port_range = "*"
...
resource "azurerm_network_security_group" "db-nsg" {
    ...
    security_rule {
        name = "ssh-rule-1"
        priority = 101
        direction = "Inbound"					#allow app-DB
        access = "Allow"
        protocol = "Tcp"
        source_address_prefix = "192.168.2.0/24"		
        source_port_range = "*"
        destination_address_prefix = "*"
        destination_port_range = "3306"				#port for DB
    }
    
    security_rule {
        name = "ssh-rule-2"
        priority = 102
        direction = "Outbound"					#allow DB-app
        access = "Allow"
        protocol = "Tcp"
        source_address_prefix = "192.168.2.0/24"		
        source_port_range = "*"
